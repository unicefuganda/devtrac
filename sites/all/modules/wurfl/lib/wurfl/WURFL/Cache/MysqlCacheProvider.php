...
class WURFL_Cache_MysqlCacheProvider implements WURFL_Cache_CacheProvider {
	...
	function put($key, $value) {
		...
		$success=mysql_query($sql,$this->_link);
		if (!$success) {
			throw new WURFL_Xml_PersistanceProvider_Exception("MySql error ".mysql_error($this->_link)."deleting $key in $this->_db");
		}

		$sql="insert into `$this->_db`.`$this->_table` (`key`,`value`) VALUES ('$key','$value')";
		$success=mysql_query($sql,$this->_link);
		if (!$success) {
			throw new WURFL_Xml_PersistanceProvider_Exception("MySql error ".mysql_error($this->_link)."setting $key in $this->_db");
		}
		return $success;
	}
