 ...
 * @version    1.0.0
 */
class WURFL_Xml_PersistanceProvider_MysqlPersistanceProvider extends WURFL_Xml_PersistanceProvider_AbstractPersistanceProvider {
	...
	protected $persistanceIdentifier = "MYSQL_PERSISTANCE_PROVIDER";
	...
	public final function initialize() {
		...
		if (mysql_error($this->_link)) {
			throw new WURFL_Xml_PersistanceProvider_Exception("Couldn't link to $this->_host (".mysql_error($this->_link).")");
		}
		
		/* Initializes link to database */
		$success=mysql_select_db($this->_db,$this->_link);
		if (!$success) {
			throw new WURFL_Xml_PersistanceProvider_Exception("Couldn't change to database $this->_db (".mysql_error($this->_link).")");
		}
		
		/* Is Table there? */
		$test = mysql_query("SHOW TABLES FROM $this->_db LIKE '$this->_table'",$this->_link);
		if (!is_resource($test)) {
			throw new WURFL_Xml_PersistanceProvider_Exception("Couldn't show tables from database $this->_db (".mysql_error($this->_link).")");
		}
		...
		if (mysql_num_rows($test)==0) {
			...
			if (!$success) {
				throw new WURFL_Xml_PersistanceProvider_Exception("Table $this->_table missing in $this->_db (".mysql_error($this->_link).")");
			}
		} 
	...
	public function save($objectId, $object) {
		...
		$success=mysql_query($sql,$this->_link);
		if (!$success) {
			throw new WURFL_Xml_PersistanceProvider_Exception("MySql error ".mysql_error($this->_link)."deleting $objectId in $this->_db");
		}

		$sql="insert into `$this->_db`.`$this->_table` (`$this->_keycolumn`,`$this->_valuecolumn`) VALUES ('$objectId','$object')";
		$success=mysql_query($sql,$this->_link);
		if (!$success) {
			throw new WURFL_Xml_PersistanceProvider_Exception("MySql error ".mysql_error($this->_link)."setting $objectId in $this->_db");
		}
		return $success;
	}
	...
	public function load($objectId) {
		...
		if (!is_resource($result)) {
			throw new WURFL_Xml_PersistanceProvider_Exception("MySql error ".mysql_error($this->_link)."in $this->_db");
		}
	...
	public function clear() {
		...
		$success=mysql_query($sql,$this->_link);
		if (mysql_error($this->_link)) {
			throw new WURFL_Xml_PersistanceProvider_Exception("MySql error ".mysql_error($this->_link)." clearing $this->_db.$this->_table");
		}
		return $success;
	}
	...
	private function _ensureModuleExistance() {
		if(!extension_loaded(self::EXTENSION_MODULE_NAME)) {
			throw new WURFL_Xml_PersistanceProvider_Exception("The PHP extension mysql must be installed and loaded in order to use the mysql.");
		}
	}
